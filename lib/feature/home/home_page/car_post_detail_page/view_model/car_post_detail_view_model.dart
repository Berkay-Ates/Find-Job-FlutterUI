...
abstract class _CarPostDetailViewModelBase with Store, BaseViewModel {
  ...
  Future sendMessage(String? message, String plate) async {
    if (message != null && message.isNotEmpty && (userHiveModel?.mail?.isNotEmpty ?? false)) {
      if (userHiveModel?.profile_img_url?.isNotEmpty ?? false) {
        await firestore.collection(plate).add(
          {
            "text": message,
            "timestamp": DateTime.now(),
            "sender": userHiveModel?.mail,
            "sender_image_url": userHiveModel?.profile_img_url
          },
        );
      }
    }
